...
class Feed extends Component {
  ...
  componentDidMount() {
    ...
    this.loadPosts();
    // const socket = openSocket('http://localhost:8080'); socket.on('posts', data
    // => {   if (data.action === 'create') {     this.addPost(data.post);   } else
    // if (data.action === 'update') {     this.updatePost(data.post);   } else if
  ...
  finishEditHandler = postData => {
    ...
    .then(fileResData => {
      ...
      let queryNameSwitcher = 'createPost';
      if (this.state.editPost) {
  ...
  deletePostHandler = postId => {
    ...
    fetch('http://localhost:8080/feed/post/' + postId, {
      ...
      console.log(resData);
      this.loadPosts();
      // this.setState(prevState => {   const updatedPosts = prevState.posts.filter(p
      // => p._id !== postId);   return { posts: updatedPosts, postsLoading: false };
      // });
    }).catch(err => {
      console.log(err);
